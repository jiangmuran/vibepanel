...
func TestResolveOrder(t *testing.T) {
	...
	tg, rest, reason := Resolve("3: hi", "b", "a", cands)
	...
	tg, rest, reason = Resolve("2: hi", "", "a", cands)
	...
	if _, _, reason = Resolve("9: hi", "", "a", cands); reason != ReasonUnknown {
		t.Fatalf("unknown handle: %q", reason)
	}
	// Two waiting sessions and no address: refused even with a focus.
	if _, _, reason = Resolve("y", "", "a", cands); reason != ReasonSeveral {
