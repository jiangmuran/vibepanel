...
// A question menu is waiting from the tool call that draws it, and the
// permission_prompt that announces it does not make it a menu one keystroke
// answers: the first key moves to the second question with the menu on screen.
func TestAQuestionMenuWaitsAndOneKeyDoesNotEndIt(t *testing.T) {
	ts, srv := newTestServer(t)
	ctx := context.Background()
	project := postJSON[store.Project](t, ts, "/api/projects", `{"path":"`+t.TempDir()+`","name":"hooked"}`)
	sess := postJSON[store.Session](t, ts, "/api/sessions", `{"projectId":"`+project.ID+`","command":["sleep","600"]}`)
	...
	report("working", `{"hook_event_name":"PreToolUse","tool_name":"AskUserQuestion","tool_input":{"questions":[
		{"question":"one","options":[{"label":"a"},{"label":"b"}]},{"question":"two","options":[{"label":"c"},{"label":"d"}]}]}}`)
	if err := srv.pollOnce(ctx); err != nil {
		t.Fatal(err)
	}
	if row, _ := srv.DB.GetSession(ctx, sess.ID); row.State != session.StateWaiting {
		t.Fatalf("after the menu was drawn: %q, want waiting", row.State)
	}
	...
	if row, _ := srv.DB.GetSession(ctx, sess.ID); row.State != session.StateWaiting {
		t.Fatalf("one key on a two-question menu: %q, want still waiting", row.State)
	}
}
