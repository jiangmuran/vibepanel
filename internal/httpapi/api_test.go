...
// A session launched as an agent never reports itself as a shell.
//
// Every command starts through a login shell so that a version manager's shims
// are on PATH, and until that shell reaches its `exec` the pane's foreground
// process is the shell. A poll landing in that window used to write "bash" into
// a row created to run claude -- which is what the session is called on screen,
// and what `TestSessionSurvivesServerRestart` caught on a CI runner slow enough
// to be inside the window when it looked. This machine measured the same shell
// at 0.00s, which is why it passed here and failed there.
func TestAStartingShellDoesNotBecomeTheSessionsName(t *testing.T) {
	for _, tc := range []struct {
		...
		{"mid-exec", []string{"claude"}, "bash", "claude"},
		{"mid-exec, absolute path", []string{"/home/x/.local/bin/claude"}, "bash", "claude"},
		{"mid-exec, sh", []string{"codex"}, "sh", "codex"},
		{"once it is running", []string{"claude"}, "claude", "claude"},

		// A session that *is* a shell has nothing more specific to say.
		{"a scratch terminal", nil, "bash", "bash"},
		{"launched as a shell", []string{"bash"}, "bash", "bash"},

		// Anything that is not a shell is the truth about the pane and wins.
		{"an agent running htop", []string{"claude"}, "htop", "htop"},
		{"nothing reported", []string{"claude"}, "", ""},
	} {
		got := runtimeCommand(store.Session{LaunchCommand: tc.launch}, tc.current)
		if got != tc.want {
			t.Errorf("%s: runtimeCommand(%v, %q) = %q, want %q",
