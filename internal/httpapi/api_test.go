...
func TestEveryAgentTheHooksPackageKnowsCountsAsInstalled(t *testing.T) {
	for _, agent := range []struct {
		...
		{"kimi",
			func(s string) error { _, err := hooks.InstallKimi(s); return err },
			func(s string) error { _, err := hooks.UninstallKimi(s); return err }},
		{"zcode",
			func(s string) error { _, err := hooks.InstallZcode(s); return err },
			func(s string) error { _, err := hooks.UninstallZcode(s); return err }},
		{"opencode",
