...
func TestRenderedPreviewServing(t *testing.T) {
	...
	t.Run("refuses a path that leaves the project", func(t *testing.T) {
		// A real page, outside the project, that exists.
		//
		// The first version of this asked for ../../etc/passwd and accepted a
		// 404, which is what filepath.Join produces for a path that escapes to
		// somewhere with nothing in it — so the test passed with the
		// containment removed. A file that is genuinely there is the only way
		// to tell "refused" from "not found", and being served is the failure.
		outside := filepath.Dir(root)
		secret := filepath.Join(outside, "outside.html")
		if err := os.WriteFile(secret, []byte("<h1>OUTSIDE</h1>"), 0o644); err != nil {
			t.Fatal(err)
		}
		t.Cleanup(func() { _ = os.Remove(secret) })

		res := get("..%2foutside.html")
		body, _ := io.ReadAll(res.Body)
		res.Body.Close()
		if res.StatusCode == http.StatusOK || strings.Contains(string(body), "OUTSIDE") {
			t.Fatalf("status %d served a file outside the project: %s", res.StatusCode, body)
		}
	...
	t.Run("refuses a page bigger than the ceiling", func(t *testing.T) {
		// A rendered preview is a click, and a click has an upper bound on what
		// it can cost. Without this an agent's 400 MB generated report is read
		// into a browser tab because somebody selected a row.
		big := filepath.Join(root, "big.html")
		if err := os.WriteFile(big, []byte(strings.Repeat("<p>x</p>", previewMaxBytes/4)), 0o644); err != nil {
			t.Fatal(err)
		}
		t.Cleanup(func() { _ = os.Remove(big) })
		res := get("big.html")
		res.Body.Close()
