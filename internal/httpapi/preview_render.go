...
func (s *Server) handleRenderPreview(w http.ResponseWriter, r *http.Request) {
	...
	// nosniff matters here more than anywhere: without it a browser that
	// disagrees about the bytes can promote them to something else, and
	// "something else" is chosen by the file. It is *not* set here, though —
	// securityHeaders sets it on every response, this route included, and a
	// second Set is a line that looks like the guard and is not. Removing it
	// from the middleware is what would matter, and the serving test asserts
	// the header on this response so that removal is red.
