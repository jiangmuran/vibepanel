...
// newRepo is a working tree on `main` with one commit in it.
//
// GIT_CONFIG_GLOBAL and GIT_CONFIG_SYSTEM point at /dev/null so the machine
// running the tests cannot change the answer: an init.defaultBranch or a
// status.showUntrackedFiles in somebody's ~/.gitconfig would otherwise make
// this suite pass here and fail in CI, or the reverse.
