...
// overridden are the environment variables run() sets for itself.
//
// Listed separately because they have to be *removed* from the inherited
// environment before being appended: exec passes the slice through untouched
// and which duplicate wins is the child libc's business, not ours.
var overridden = []string{
	...
	"GIT_PAGER=cat",
	"PAGER=cat",
	// Inherited GIT_DIR or GIT_WORK_TREE would point every invocation at
	// whatever repository the panel was started from rather than at cmd.Dir.
	// Nothing sets these today; a systemd unit written by hand could.
	"GIT_DIR=",
	"GIT_WORK_TREE=",
	"GIT_INDEX_FILE=",
}
...
func env() []string {
	drop := make(map[string]bool, len(overridden))
	for _, kv := range overridden {
		k, _, _ := strings.Cut(kv, "=")
		drop[k] = true
	}
	base := os.Environ()
	out := make([]string, 0, len(base)+len(overridden))
	for _, kv := range base {
		k, _, _ := strings.Cut(kv, "=")
		if drop[k] {
			continue
