...
func TestBinaryFrameLayoutMatchesTheClient(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		// wire.ts declares four constants and this pins three. The fourth,
		// EXIT_VANISHED, is pinned by TestExitVanishedMatchesTheStore in
		// internal/httpapi instead, for two mechanical reasons: `declared` parses
		// `0x..|\d+` and cannot match `-1`, and the value is a store constant, so
		// the comparison wants int64 rather than uint64. That package already
		// imports store and already reads this same file.
	} {
