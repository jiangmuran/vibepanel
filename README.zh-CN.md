...
还有一个我自己天天在用的功能：把会话接到手机上的聊天软件里。Telegram、飞书、个人微信都行，全部是单聊。哪个 agent 停下来要你允许一条命令、或者在问你问题，手机上就会收到一张卡片，卡片开头就是会话编号，直接回「3号可以」「3: 再加个测试」就送回那个会话，Telegram 和飞书上还有允许/拒绝按钮。随手发的「嗯，好的。」「第二个在干嘛」也听得懂，出门在外也不耽误 agent 干活。

隐私和安全这件事我是认真的：三个聊天接入全部是自己写的，没有引入任何第三方 SDK，和面板打包在同一个二进制里；微信登录的二维码在你的浏览器里本地生成，截图用的字体也打包在里面，不用临时去网上下载任何东西。**在你配置并开启某个通道之前，聊天功能不会连接任何外部服务**，这一点有测试盯着。第一次开启时，面板会明确告诉你哪些内容会经过哪家的服务器，你点了确认才会开始。陌生人给机器人发消息只会拿到一个配对码，必须由你在面板里输入才算配对；每一次「允许」只对你真正看过的那条请求生效，旧卡片永远不会误批新命令。
...
[docs/features.zh-CN.md](docs/features.zh-CN.md)，包括[手机上的聊天](docs/features.zh-CN.md#手机上的聊天)：Telegram、飞书、个人微信，回复直接送回会话。
⚠️ 此内容由AI编写 ⚠️
