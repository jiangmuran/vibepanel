...
<sup>左栏是全部会话，按项目分组，卡住的排在最上面。三角：停下来问话了。圆点：还在干活。
对勾：跑完了。叉：进程非零退出。中间下方那一条是挂在当前会话下的便签终端。右栏是这个项目的
文件树，底下是机器的 CPU、内存和磁盘。</sup>
...
**设置 → 只读分享链接**生成一个可以挂在另一块屏幕上的地址：`https://<面板>/share/<token>`。
它只打开一块看板，别的什么都打不开：没有终端，没有写入，没有文件浏览，
也没法用一个链接再生成一个。

看板长什么样是生成链接时选的，二十四个起手式按「打算挂在哪块屏上」分成手机、电脑、
墙上的屏、4K 大屏四组。其中两个是按「屋里坐着谁」起的名。**给领导看**是那个 4K 的：
最上面是大数字——今天烧了多少 token、几个 agent 在干活——中间是一条会动的机器曲线，
下面是铺满剩余空间的细网格。**给客户看**会自己限定到一个项目并关掉名字，因为这里出错的
样子是：客户在自己面前那块屏上读到了另一个客户的项目名。

起手式只是起手式，不是模式。每个组件都能挪、能改宽改高、能换成别的数、能换个维度拆
（按 agent、按项目、按模型、按天、按月），一共三十七种组件可以往上加。网格是十二列宽、
四行高，所以一个组件可以占三分之一面墙，也可以比旁边的方块高三倍。看板还可以设成**铺满**
整屏而不是往下流。同一个看板照样随屏幕收窄，在手机上是摘要，在 4K 屏上是排好的一整面墙。

**改布局不用走到大屏跟前。** 在设置页里改，屏幕在下一次轮询时自己跟上。编辑器旁边是实时
预览，按正在显示这个链接的那块屏的比例画；每一行还写着此刻有几块屏打开着它。给链接**上锁**，
在解锁之前它的看板改不了——这就是防止手滑改掉客户正在看的那一块。**备注**（比如
「三楼会议室那块屏」）会同时出现在屏幕上和设置页那一行。

数据是面板本来就知道的那些：状态和已经维持了多久、每个会话的 CPU 和内存、机器、待办完成度，
以及 agent 自己记的 token 账。只有 token，不折算成钱：价格按模型、按档位、按时间都不一样，
拿过期价目表算出来的金额，是一个看起来很确定的错数。

链接可以限定成整个面板、一个项目或一个会话，范围由服务端按链接自己那行强制执行。
详细程度分两档：**只有数量和状态**只显示形状和数字，一个字都不带；**连名字一起**会把会话
标题和项目名也发出去。两档都不会发送路径、工作目录、命令行、主机名和面板自己的 id。
看板、备注和锁可以事后改，档位和范围不行——等想改的时候地址早就躺在别人邮件里了，
