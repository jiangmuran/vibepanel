...
- **一个看板生成器，给旁边那块屏幕。** 分享链接打开的是你自己拼的**看板**：十九个起手式，
  按「谁在看」分组 —— 隔着房间问「有没有事要我处理」、把所有会话铺满一整块电视、只看机器、
  今天烧了多少、一年 53 周的格子图、一个数字占满整屏、三页轮着放。每一个都能再逐个组件改。
  链接可以是整个面板，也可以只针对一个项目或一个会话 —— 后者就是发给一起干活的人的那种。
  它只能打开那一个页面，默认连名字都不显示。
...
一共四种装法，区别不在于「你有多熟练」，而在于**这台机器是谁的**、以及**什么必须活下来**。
...
tmux 活得过**面板**，活不过**机器**：tmux server 就是一个普通进程，那 20000 行回滚记录
就在它的内存里，`reboot` 两样都带走。

面板会把重建一个会话需要的东西记下来——建它时用的 argv、它的目录、名字和排序位置，以及
每 30 秒抓一次、关机时再抓一次的一份有界的回滚记录。重启之后它会**问你**要不要恢复，一个
或者全部，并且把每个会话将要执行的命令和目录明明白白列出来。也可以给某个会话单独打开
「开机自动恢复」——默认是关的，因为开机瞬间同时拉起二十几个 agent 比让你点一遍更糟。

**进程回不来。** agent 的上下文活在那个进程和它跟服务商的一段对话里，断电两样都没了；
重跑命令启动的是一个全新的、什么都不记得的 agent。恢复出来的 pane 会在旧回滚记录和新
进程之间打一条横幅说明这件事，会话本身也会留一个 `restored` 标记——因为横幅会滚走，事实
不会。

正常 `reboot` 或 `systemctl stop` 一行输出都不会丢。断电最多丢半分钟——那种情况下没有
「关机」这一步给最后一次抓取搭车。
...
`https://<面板>/share/<token>`。它只打开那一个页面，除此之外什么都打不开。

那个页面显示什么，是你在生成链接时选的**看板**。十九个起手式，按「谁站在屏幕前面」分组，
而不是按「数据来自哪张表」分组：

| | |
|---|---|
| 自己正在干活 | 总览 · 有没有事要我处理 · 等待队列 · 会话方阵 · 只看等着回话的 · 全都要 |
| 墙上的那块屏 | 四个数字加一个钟 · 一个数字占满 · 现在有多忙 · 三页轮播 |
| 管机器的人 | 只看机器 · 有没有出事 |
| 老板和领导 | 花了多少 vs 做出来什么 · 钱去哪了 · 数字少字大的那个 · 这一年 |
| 盯着一件事看 | 按项目 · 哪个模型在干活 · 今天花了多少 |

起手式只是起手式，不是模式。上面每个组件都能挪位置、改宽度、换成别的数、换个维度拆
（按 agent、按项目、按模型、按天、按月），也能直接删掉；还能从二十一种组件里再加。看板
跟链接一起存着，所以换台机器打开还是同一个看板；宽度会随屏幕自动收窄，同一个看板在手机上
是摘要，在电视上就是铺开的四十块。生成之后还能改看板，地址不变。

背后的数据就是面板本来就知道的那些：会话状态和已经维持了多久、每个会话的 CPU 和内存、
机器、每个项目的待办完成度，以及 agent 自己记下来的 token 消耗 —— 今天对昨天、本月对上月、
今天到现在平均每小时多少、按 agent / 按项目 / 按模型拆开，还有 53 周的格子图。只有 token，
不折算成钱：价格按模型、按档位、按时间都不一样，拿一张过期的价目表算出来的金额，是一个
看起来很确定的错数。也不统计代码行数 —— 面板从来不读你的仓库。
...
会话标题和项目名也发出去。但两档都绝不会发送路径、命令行、主机名、待办条目的内容，以及
面板自己的 id —— 项目路径会暴露客户名和 home 目录结构，命令行则带着 agent 启动时的全部
参数。如果那块屏幕就在你身后、别人来回走动，那就别动默认值。

链接还可以**限定范围**：整个面板、某一个项目，或者某一个会话。限定到项目的那种，就是发给
一起做这个项目的人的：它只看得到这个项目的会话、这个项目的花费、这个项目的待办计数，别的
一概看不到。范围是服务端按链接自己那行记录强制执行的；项目后来被删掉的话，链接显示的是
「什么都没有」，而不是退回去显示全部。

看板可以事后改，档位和范围不行。等你想改的时候，那个地址早就躺在某封邮件里或者被敲进了
一台电视，把它能看到的东西改宽，拿着地址的人根本不会知道。要换档位，就换一个链接。
