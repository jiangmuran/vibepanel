...
- **文件**：浏览、点一下下载。拖到文件树上或者直接拖到终端上就是上传，文件落在会话旁边，
  ...
  长文本按 256 KiB 或 4000 行截断。

  `.html` 和 `.svg` 会真的画出来：跑在一个 origin 不透明、默认不执行脚本、而且策略上
  完全不允许联网的 frame 里 —— 项目里的页面碰不到面板的 cookie，也不会趁你看的时候
  去拉任何东西。标题栏里永远有一个**源码**开关；脚本是按文件的开关，每次打开都从关开始。
- **仓库**：工作区现在什么样 —— 在哪个分支、和上游差多少、有多少没提交、最近十五个提交，
  全部从磁盘读，不需要任何凭据。在别的 worktree 里的会话单独一行，带自己的分支。
  有一个按钮去问 GitHub 有哪些开着的 PR，并按分支名和上面那些行对上；它需要面板环境里的
  `GITHUB_TOKEN` 或 `GH_TOKEN`，只有按了才会联网，没有 token 时这个 tab 的其余部分照常。
...
一下——每次轮询都会重新读一遍链接那行。你拖的那块画布，按**正在显示这个链接的那块屏**的
比例画，数据也是那块屏此刻看到的真数据；每一行还写着此刻有几块屏打开着它。给链接**上锁**，
在解锁之前谁都改不了它的看板——这就是防止你在一堆链接里手滑改掉客户正在看的那一块。

给链接写个**备注**——「三楼会议室那块屏」「给客户看的」——它会同时出现在屏幕上和设置页那一行。
两档详细程度都会显示：它是你写给站在屏幕前那个人的话，不是面板自己的字。
...
看板、备注和锁都可以事后改，档位和范围不行——等你想改的时候地址早就躺在别人邮件里了，把它
