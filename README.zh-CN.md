...
看板长什么样，是生成链接时选的。二十四个起手式，按「打算挂在哪块屏上」分组：

| | |
|---|---|
| 手机 | 一列三块，站着看四秒 |
| 电脑 | 总览 · 有没有事要我处理 · 等待队列 · 全都要 · 只看机器 · 有没有出事 · 花了多少 vs 做出来什么 · 钱去哪了 · 按项目 · 哪个模型在干活 · 今天花了多少 |
| 墙上的屏 | 四个数字加一个钟 · 一个数字占满 · 现在有多忙 · 三页轮播 · **token 实时消耗** · 会话方阵 · **给客户看** · 数字少字大的那个 · 这一年 |
| 4K 大屏 | **给领导看** · 走廊大屏 |

其中两个是按「屋里坐着谁」起的名。**给领导看**是那个 4K 的：最上面是大数字——今天烧了多少
token、几个 agent 在干活——中间是一条会动的机器曲线，下面是铺满剩余空间的细网格。
**给客户看**会自己限定到一个项目并关掉名字，因为这里出错的样子是：客户在自己面前那块屏上
读到了另一个客户的项目名。

起手式只是起手式，不是模式。每个组件都能挪、能改宽、能改高、能换成别的数、能换个维度拆
（按 agent、按项目、按模型、按天、按月），一共**三十七种组件**：大数字、仪表盘、迷你走势、
近十五分钟的机器曲线、分层的 token 柱状图、排行、会话方阵、等待时长图、一年的格子，还有
排版要用的留白、分隔线、小标题和这块屏自己的名字。

网格是**十二列宽、四行高**，所以一个组件可以占三分之一面墙，也可以比旁边的方块高三倍。
层次就是从这个尺寸比例来的：每块方块一样大的屏幕是仪表盘，不是给人看的大屏。看板还可以
设成**铺满**整屏而不是往下流——没人会去滚一台电视。同一个看板照样随屏幕收窄，在手机上是
摘要，在 4K 屏上是排好的一整面墙。

**改布局不用走到大屏跟前。** 在自己电脑上的设置页里改，两秒之内屏幕自己就跟上了，不用碰它
一下——每次轮询都会重新读一遍链接那行。编辑器旁边是实时预览，按**正在显示这个链接的那块屏**
的比例画；每一行还写着此刻有几块屏打开着它。给链接**上锁**，在解锁之前谁都改不了它的看板——
这就是防止你在一堆链接里手滑改掉客户正在看的那一块。
