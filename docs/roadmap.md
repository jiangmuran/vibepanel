...
- [ ] **U6 webhook 通知（微信 / 安卓 / iOS）+ 群猫娘。** 现在只有 PWA 的浏览器通知，
      需要面板在前台或装成 App。真正「推到手机上」的那条路没有。
...
- [ ] **V1 目录选择器完全重构。** 「仍然得完全重构」——交互先定、像素后定。*(子任务进行中)*
- [ ] **V2 面板 chrome：动效与统一。** 右边 tab 切换没有动画；面板收窄时「非常僵硬地多一个
      按钮」——**控件不该出现和消失**，能变的是呈现方式而不是控件的集合；底部那一排控件
      「像拼起来的」。*(子任务进行中)*
- [ ] **V3 右栏可拖拽分 pane。** 拖一个 tab 出来把右栏分成多个 pane（**在面板这一层，不是
      tmux 层**），可以拖回去合并，有一键复原；每个 pane 有自己的状态。同时要求
      **更密、更有信息量，但仍然优雅**。布局存在 **client 端**（localStorage，按视口档位分开），
      换显示器、换设备、多设备、手机**都不自动同步**。*(并入 V2 的子任务)*
- [ ] **V4 README 重写。** 「写得像屎」，而且「**AI 味太重，不像介绍项目，像在跟我汇报工作**」。
      *(子任务进行中)*
- [ ] **V5 看板生成器。** 只读链接不该只用来看 agent；**十种以上不同用途、可自定义**，
      从头构思。*(子任务进行中)*
- [ ] **V6 一行命令安装。** 直接从 GitHub 拉、macOS/Linux 都行、tmux 缺失或过旧时提示并
      带自动安装流程、**默认推荐装成系统服务**、外加一个管理命令。*(子任务进行中)*
