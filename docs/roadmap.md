...
## 第八批（2026-09-14，分享只剩页面）

- [x] **Y1 只读看板整个删掉，分享只剩分享页面。** 「只读文档不都是要删了的功能」，而且设置里
      看板编辑器和页面列表挤在同一节、每个链接还有一个「显示：看板/页面」的下拉，谁都看不懂。
      删掉的是看板编辑器、三十个预设、组件注册表、`/api/share/{token}/dashboard`、SPA 里的
      看板根、`board-check`。**已经发出去的地址不删**：启动时每个老链接按它看板最接近的内置
      模版（产出 / token 消耗 / 手机速览 / 会话墙）建一个页面、发布 v1、原地址指过去，详细
      程度、范围、屏幕名和过期时间都不变。W7 里「todo 路由留着是因为墙上的看板在调」这条理由
      变成了「分享页面还能数 todo」。
- [x] **Y2 页面目录放进数据目录。** 「数据直接放在 ~/ 不太好」——默认
      `<数据目录>/pages/page-<slug>`，和 `pasted/` 挨着，项目名也是 `page-<slug>`。「项目丢了
      也可以通过在面板打开」：目录或项目没了，设置里点**恢复并打开**，把已发布的版本写回去。
- [x] **Y3 发版和看链接说清楚。** 设置 → 分享是一个列表：页面 → 打开 / 发布 / 版本 → 下面挂
      链接；链接只在创建时显示一次地址，之后用「查看」（眼睛）开一个十五分钟的临时副本看它
      现在显示什么。失效的地址不再落到面板的 SPA 上，只给一张写着「链接已失效」的静态页。
