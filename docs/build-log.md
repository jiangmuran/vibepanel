...
- **`docs/api.md` had a merge conflict committed into it** — a pair of markers
  swallowing the boundary between the update section and the share-link one.
  Resolved by keeping both sides, which is what the two halves were. Worth
  knowing that `TestTheAPIDocCoversEveryRoute` passed the whole time: it reads
...
### The VNC viewer is off unless somebody asks, and off means absent

The user's instruction was 「把 vnc 模块默认禁用吧 暂时用不上 别占用右栏的地方」, and
the interesting half is the first clause rather than the second. Not rendering
the tab would have satisfied the request as typed. It would also have left the
routes there.

So `--vnc` gates `registerVncRoutes` itself. A panel nobody turned it on for
answers 404 to every VNC route, and the settings section is hidden because a
form posting into a 404 is worse than no form — not because hiding it protects
anything. This is the same shape as red line 8: what a caller can do is decided
by which routes exist, not by a flag a handler is supposed to remember to
consult. It is the one place in the panel that opens an outbound TCP connection
on a browser's say-so, which is exactly the capability that should not be
sitting behind a hidden control.

Two mutations, and **the second is the one worth writing down**:

- Removing the gate so the routes always register — caught by
  `TestTheVncRoutesDoNotExistUnlessAskedFor`, which walks the router rather
  than probing a URL somebody would have to remember to add to a list.
- **Flipping the flag's own default to `true` — passed everything.** Every test
  in the tree went green, because the routes then exist and the fourteen tests
  that exercise them are *happier*. That is the mutation that ships a
  capability nobody asked for with a clean build, and nothing was asserting the
  default at all. `TestTheVncViewerIsOffUnlessAskedFor` in `internal/config`
  now does, in both directions.

`TestTheAPIDocCoversEveryRoute` had to learn about this too: the doc documents
the VNC routes, and a walk of a default-built router no longer finds them. The
test server turns every optional feature on, so the doc check sees the whole
surface — a route that only exists behind a flag is still a route somebody will
write code against, and a check that ran against the default build would let
that section of `docs/api.md` rot the moment nobody was running with `--vnc`.
The doc says the flag is required, at the top of the section.

Two files had each independently invented `type scanner interface{ Scan(...) }`
— `internal/store/vnc.go` and `internal/store/profiles.go`, written by
different work on different branches. The package stopped compiling the day
they met, which is the compiler catching what no reviewer of either branch
could have. One definition now, in `store.go`.

Three test files were asserting the tab list by writing it out. Adding a sixth
tab turned seven tests red, none of them about the sixth tab: the pane fixtures
spelled out which tabs were left in the first pane after a drag, and the arrow
tests named `tokens` when what they meant was "the last one". They derive both
now. The one literal that stays a literal is `PANEL_TABS`'s order — that one is
the guard, and having to edit it is the point.
...
### Optional chaining swallowed every copy on a LAN address

Reported: 「我明明复制上了 但是没有出现在设备剪切板里面」, from a panel served
over plain http on `192.168.8.4`.

`navigator.clipboard` does not exist on a non-secure origin, and this was
written five times in five files with three different answers between them. Four
had the same wrong one:

```ts
navigator.clipboard?.writeText(text).catch(fallback)
```

It reads as "write it, and if that fails do this". It is not. **Optional
chaining short-circuits the entire chain**, so with no clipboard object the
expression is `undefined` and `.catch` is never reached. Not a silent failure
with a fallback that also failed — the fallback was never called. Selecting text
put nothing on the clipboard and offered nothing, and the terminal's own comment
three lines above it described a fallback that could not run.

The terminal is the least of it. The other three were **ShareLinks**,
**ApiTokens** and **Settings** — every one a "copy this, you will not see it
again" button. A share token and an API token are readable exactly once; a copy
that fails silently there costs the token.

One module now, `web/src/clipboard.ts`, with two functions because the two
callers are genuinely different: `copyText` for text arriving with nobody
touching the page (OSC 52 from tmux), which can only report failure so the shell
can offer a click; and `copyTextInGesture` for a pointerup or a click, where
`document.execCommand('copy')` is allowed and is the only thing that works on an
insecure origin. That one is synchronous on purpose — `execCommand` has to run
in the same task as the gesture that authorised it, and awaiting the modern API
first spends the authorisation before the fallback can use it.

The guard is a source scan rather than a behaviour test: the failure was not in
what a function did but in how one was called, and vitest runs in `node` here
with no DOM to render a call site into. It fails on `navigator.clipboard?.` in
any file, and on any file other than `clipboard.ts` touching
`navigator.clipboard` at all. Comments are stripped before scanning, because
Terminal.tsx quotes the broken line to explain it and quoting a mistake is not
making one.

Two mutations, one survived and was **deleted rather than tested**:
`copyText`'s `if (!navigator.clipboard) return false` changes nothing, because
reaching through an undefined clipboard throws and the throw lands in the same
`catch` with the same answer. A branch whose removal changes no behaviour reads
as a guard without being one.

### A red that is a shape, and a red that is a word

`--vp-state-crashed` (#ff3b30) is the state dot's red, and a dot is a shape as
well as a colour, so it never had to pass a contrast bar. The repo line started
writing the word **conflict** in it, and render-check measured **3.34:1 against
a required 4.5** in light. `--vp-danger-ink` is the same alarm dark enough to be
read as text, defined once in each of the three theme blocks — root, the
...
| said | is |
|---|---|
| "Thirty starting points" | twenty-nine |
...
Two of the twenty-nine are both called *for leadership*, one composed for a wall
