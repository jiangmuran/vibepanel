...
built, and the plugin half turned out on inspection to be four different
features wearing one word.

The conclusion, up front, so nobody has to read to the end for it:

> **For the thing that was asked for — a harness that manages every session —
> the API already does it, and a plugin system would do it worse.** What does
> not exist is a way to *package* such a harness so it is a file you drop in
> rather than a daemon you supervise. That is a distribution problem, not a
> runtime one, and the two should not be solved with the same mechanism.

## What a plugin could mean here, and what each one really is

Four things get called a plugin. They have nothing in common except the word.
...
anything changing — the coalesce window in `internal/ws/hub.go`, not a poll
interval. From there it can create sessions, restart them, rename them, mark
state, read and write notes and todos, read the machine, and type into any
...
**Already covered. Not a plugin.**

### 2. "Send an event somewhere when something happens"

Also done, and by a feature nobody has connected to this question:
`POST /api/settings/webhooks` fires an outbound HTTP request on a state
transition, with `{state}`, `{session}`, `{project}`, `{url}` and `{time}`
...
The one thing missing is that a webhook cannot read anything back — it is fire
and forget. But its target holds no credential and needs none: a program that
wants to *do* something in response holds an API token and calls back. The two
halves exist; what does not exist is a paragraph saying so, which this is.

**Already covered by two features that were never introduced to each other.**

### 3. "Add something to the interface"

This is the one that sounds most like a plugin and is the one to refuse
hardest.
...
person signed in has, granted to a file. There are exactly two shapes:

- **Same origin.** The plugin has the terminal. Calling that a plugin API is a
  way of not saying "arbitrary code as you, in the page".
- **An iframe on another origin.** The plugin has nothing, and to get anything
  it needs an API token — which is section 1 again, with a rectangle.

Neither is a middle. A `postMessage` bridge is the first one with a list of
allowed messages, and the list is a thing somebody extends on a Tuesday.

**Refused.** If a panel tab is wanted, it is a pull request, not a plugin.

### 4. "Ship it as a file, not a daemon"

Here is the real gap, and it is not about capability at all.
...
- **A plugin reaches the HTTP API and nothing else.** Not the database, not the
  tmux socket, not `internal/*`. It is a process with a token, which means
  every scope question is the API-token question and gets answered once.
- **It gets its own token**, minted at install and revoked at uninstall, so
  ...
  in the audit log.
- **The default is a token that can read and nothing else.** Today API tokens
  are not scoped at all — one token does everything — so this is work that
  would have to happen first, and it is the honest reason section 4 is not a
  small change after all.
- **It never gets the tmux socket.** Red line 1 is that the panel touches one
  socket and nothing else touches it. A plugin holding it is a plugin that can
  `kill-server`.

## Installing one, said plainly

A plugin under section 4 **is a way to run arbitrary code as the user who runs
the panel**. That is not a flaw to be mitigated; it is what "run my script when
a session finishes" means. The only wrong thing to do about it is to be quiet.
...
That scar is the whole design rule, and it is why the subprocess shape is the
right one:

- **A separate process cannot block the poller.** There is no call to wait on.
  The strongest coupling available is a webhook, which is already fire and
  forget with the goroutine to prove it.
...
- **A loop is the operator's problem, bounded.** The process gets a memory and
  CPU ceiling from the same unit the panel does; it is not the panel's job to
  schedule somebody's script.
- **Its output is bounded and dropped oldest-first**, like a session's ring
  buffer. A plugin printing in a loop must not fill the disk the projects live
  on.
...
- **Nothing else is versioned, because nothing else is exposed.** This is the
  main argument for the subprocess shape over an in-process one: a WASM ABI or a
  Go plugin interface is a second surface with its own compatibility story, and
  the panel would then have two. `docs/api.md` is already checked against the
  router in both directions; a second contract would not be.

## What was considered and rejected for the runtime

- **Go's `plugin` package.** Requires cgo, and `CGO_ENABLED=0` must keep
  working — that is a stated convention, not a preference. It also requires the
  identical toolchain and dependency graph, which for a project distributed as
  release binaries means it works for nobody.
...
it will be. That is the feature, and it is finished; it was only ever missing a
name.
...
token, both of which are ordinary, and neither of which is a plugin system.
