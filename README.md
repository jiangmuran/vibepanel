...
The dashboard draws a **board**, chosen when the link is made. Twenty-four
presets come with it, grouped by the screen they were composed for: a phone, a
laptop, a wall, a 4K wall. Two are named for who is in the room. *For leadership* is the 4K one:
today's tokens and how many agents are working, over a machine line that moves,
over a fine grid. *For a client* scopes itself to one project and turns names
off, because the failure there is a customer reading another customer's project
name off the screen in front of them.
...
day, month — and there are thirty-seven kinds to add: single figures, gauges,
sparklines, a machine line over the last fifteen minutes, stacked token bars,
ranked breakdowns, session tiles, a dwell timeline, the year as a grid of days,
and the furniture a composed screen needs. The grid is twelve columns wide and
four rows tall, so a widget can
be a third of a wall or three times the height of the tiles beside it. A board
can also be set to fill the screen rather than flow down it. The same stored
board still collapses with the viewport, so it is a summary on a phone and a
composed wall on a 4K screen.

**Changing a board does not mean walking to the wall.** It is edited from the
settings page and the screen follows on its next poll. Beside the editor is a
live preview drawn at the shape of the screen actually showing the link, and
each row says how many screens have it open. **Locking** a link freezes its
board until it is unlocked, which is what stops the one a customer is watching
from being rearranged by accident. A **remark** — "the screen in meeting room
three" — appears on the screen as well as in the settings row.
