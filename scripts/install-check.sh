...
if command -v script >/dev/null 2>&1; then
  ...
  # actually meets.
  printf '1\n1\nn\ny\n' | timeout 90 script -qec "cd '$REL' && HOME='$HOME_DIR' \
...
# answer nobody sends does not fail, it stalls -- and a run killed by the outer
# timeout leaves a truncated log whose FAIL count is zero, which reads exactly
# like a pass. That happened, and the "0 FAIL" it produced was believed twice.
#
...
if command -v script >/dev/null 2>&1; then
  pty_run() { # pty_run <answers>
    PTY_LOG="$WORK/pty.log"
    printf '%b' "$1" | timeout 90 script -qec "cd '$REL' && HOME='$HOME_DIR' \
      VIBEPANEL_DESTDIR='$HOME_DIR/root' VIBEPANEL_SYSTEMCTL='$SCTL' \
      VIBEPANEL_LAUNCHCTL='$LCTL' VIBEPANEL_PLATFORM=linux \
      VIBEPANEL_PKG_RUNNER='$PKGREC' VIBEPANEL_CLAUDE_BIN=none \
