...
function Ago({ when }: { when: number }) {
  const { value, unit } = agoParts(when, Math.floor(Date.now() / 1000))
  ...
  return (
    <div className="px-3 py-2" data-testid="git-panel">
      ...
      <div className="mt-3 border-t border-hairline pt-2">
        <div className="flex items-baseline gap-2">
          ...
          {info.github && info.tokenSet && (
            <button
              ...
              data-testid="git-ask"
              title={t('git.ask')}
              className="vp-press inline-flex shrink-0 items-center gap-1 rounded-md border border-hairline px-2 py-0.5 text-vp-xs text-ink-2 transition-colors duration-200 ease-vp hover:bg-surface-2 hover:text-ink disabled:opacity-50"
            >
        ...
        {/* The one sentence. Said once, here, where the only button that leaves
            the machine is — not repeated anywhere else and not explained. */}
        <p data-testid="git-offline" className="mt-1 text-vp-xs text-ink-2">
          {t('git.onlyWhenAsked')}
        </p>
        {!info.github && (
